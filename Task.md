...
- `[x]` **Penyisipan Kolom & Pewarnaan Dinamis Kode BOM:** Menambahkan kolom `Kode BOM` di antara kolom `Gambar` dan `Nama Barang`. Mengimplementasikan logika pewarnaan lencana (*badge*) berbasis awalan string: `FG-1` (Oranye), `FG-2` (Biru), dan `FG-4` (Merah). Untuk menjamin keterbacaan (*readability*) tinggi pada *Dark Mode* maupun *Light Mode*, latar belakang *badge* menggunakan teknik *opacity* pudar (RGBA 10%) dengan warna *font* murni. Menerapkan utilitas responsif `d-none d-md-table-cell` pada kolom `URL`, `Klik`, dan `Status` agar tabel tidak berdesakan saat diakses dari ponsel. Selain itu, judul disingkat menjadi `Manajemen Cari`, teks `Tampilkan:` dihilangkan beserta blok menu *dropdown* (*select*) halamannya, dan ikon pensil di tombol Edit disembunyikan khusus pada tampilan *Mobile*.
